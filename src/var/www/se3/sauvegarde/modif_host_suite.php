...
if (($HostServer != "") and ($XferMethod=="archive") and ($ArchiveDest!="")) {
	...
	echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0; URL=new_host_valid.php?HostServer=".htmlspecialchars($HostServer, ENT_QUOTES, 'UTF-8')."&TypeServer=".htmlspecialchars($TypeServer, ENT_QUOTES, 'UTF-8')."&XferMethod=".htmlspecialchars($XferMethod, ENT_QUOTES, 'UTF-8')."&ArchiveDest=".htmlspecialchars($ArchiveDest, ENT_QUOTES, 'UTF-8')."&ArchiveSplit=".htmlspecialchars($ArchiveSplit, ENT_QUOTES, 'UTF-8')."&dhcp=".htmlspecialchars($dhcp, ENT_QUOTES, 'UTF-8')."\">";
        exit;
}
			

if (($HostServer == "") or ($TypeServer == "") or ($XferMethod == "") or ($dhcp == "") or (($Share == "") and ($ShareName1 =="") and ($ShareName2 == "")) or ($defo == "")) {
	echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0; URL=modif_host.php?HostServer=".htmlspecialchars($HostServer, ENT_QUOTES, 'UTF-8')."&TypeServer=".htmlspecialchars($TypeServer, ENT_QUOTES, 'UTF-8')."&XferMethod=".htmlspecialchars($XferMethod, ENT_QUOTES, 'UTF-8')."&Share=".htmlspecialchars($Share, ENT_QUOTES, 'UTF-8')."&ShareName1=".htmlspecialchars($ShareName1, ENT_QUOTES, 'UTF-8')."&ShareName2=".htmlspecialchars($ShareName2, ENT_QUOTES, 'UTF-8')."&defo=".htmlspecialchars($defo, ENT_QUOTES, 'UTF-8')."&Compte=".htmlspecialchars($Compte, ENT_QUOTES, 'UTF-8')."&PassWord=".htmlspecialchars($PassWord, ENT_QUOTES, 'UTF-8')."&AclName=".htmlspecialchars($AclName, ENT_QUOTES, 'UTF-8')."&LdapName=".htmlspecialchars($LdapName, ENT_QUOTES, 'UTF-8')."&MysqlName=".htmlspecialchars($MysqlName, ENT_QUOTES, 'UTF-8')."&Secrets=".htmlspecialchars($Secrets, ENT_QUOTES, 'UTF-8')."&dhcp=".htmlspecialchars($dhcp, ENT_QUOTES, 'UTF-8')."&BackupFilesExclude=".htmlspecialchars($BackupFilesExclude, ENT_QUOTES, 'UTF-8')."&err=".htmlspecialchars($err, ENT_QUOTES, 'UTF-8')."\">";
	exit;
}	
	...
	include "entete.inc.php";
  	...
  	if ((is_admin("computers_is_admin",$login)=="Y") or (is_admin("inventaire_can_read",$login)=="Y")) {
		...
		echo "<table align=center width=\"70%\" border=1 cellspacing=\"0\" cellpadding=\"0\">";
  		...
  		echo "<input type=\"hidden\" name=\"Secrets\" value=\"".htmlspecialchars($Secrets, ENT_QUOTES, 'UTF-8')."\" />";
